jQuery(function($){
	...
	$('.acf-buckets .choices .list li, .acf_buckets li').live('mouseover', function(){
		var pID = $(this).find('.acf-rel-item').data('id');
        var url = $('.new-bucket-button').attr('href');
		if ($(this).find('span.edit').length==0){
			$(this).prepend('<span class="edit" data-url="' + url.replace('&TB_iframe=1', '&bucket_id=' + pID) + '&TB_iframe=1"><span class="dashicons dashicons-edit"></span></span>')
		}
	});

    //http://s.hileclients.com/roots/wp-content/plugins/buckets/admin/add_bucket.php?post_type=buckets&popup=true&TB_iframe=1&width=920&height=543
