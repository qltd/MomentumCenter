(function($){
    ...
    acf.fields.buckets = {
        ...
        fetch : function(){
            ...
            $.ajax({
                ...
                dataType        :   'json',
                data            :   $.extend({
                    action      :   'acf/fields/buckets/query_posts',
    ...
    */
    ...
    $(document).on('acf/setup_fields', function(e, el){
        ...
        $(el).find('.acf_buckets').each(function(){

            acf.fields.buckets.set({ $el : $(this) }).init();
    ...
    */
    ...
    $(document).on('change', '.acf_buckets .select-post_type', function(e){

        // vars
        var val = $(this).val(),
            $el = $(this).closest('.acf_buckets');
        ...
        // fetch
        acf.fields.buckets.set({ $el : $el }).fetch();

    });


    $(document).on('click', '.acf_buckets .relationship_left .relationship_list a', function( e ){

        e.preventDefault();

        acf.fields.buckets.set({ $el : $(this).closest('.acf_buckets') }).add( $(this) );

        $(this).blur();

    });

    $(document).on('click', '.acf_buckets .relationship_right .relationship_list a', function( e ){

        e.preventDefault();

        acf.fields.buckets.set({ $el : $(this).closest('.acf_buckets') }).remove( $(this) );

        $(this).blur();

    });

    $(document).on('keyup', '.acf_buckets input.relationship_search', function( e ){
        ...
        // fetch
        clearTimeout( acf.fields.buckets.timeout );
        acf.fields.buckets.timeout = setTimeout(function(){
    ...
    $(document).on('keypress', '.acf_buckets input.relationship_search', function( e ){
